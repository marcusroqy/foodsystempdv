...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6 md:p-12 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-full max-h-[800px] flex overflow-hidden animate-in zoom-in-95 duration-200">

                {/* Sidebar de Configurações */}
                <div className="w-64 bg-gray-50 border-r border-gray-200 flex flex-col">
                    <div className="p-6 border-b border-gray-200">
                        <h2 className="text-xl font-bold text-gray-900">Configurações</h2>
                        <p className="text-sm text-gray-500 mt-1">Gerencie sua conta e sistema</p>
                    </div>

                    <div className="flex-1 overflow-y-auto p-4 space-y-1">
                        <button
                            onClick={() => setActiveTab('profile')}
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'profile' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-100'}`}
                        >
                        ...
                        {(user?.role === 'OWNER' || user?.role === 'MANAGER') && (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'company' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-100'}`}
                            >
                                <Building className="w-5 h-5" /> Dados da Empresa
                            </button>
                        )}

                        <button
                            onClick={() => setActiveTab('security')}
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'security' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-100'}`}
                        >
                        ...
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'notifications' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-100'}`}
                        >
                        ...
                        {user?.role === 'OWNER' && (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'billing' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-100'}`}
                            >
                                <CreditCard className="w-5 h-5" /> Assinatura (SaaS)
                            </button>
                        )}
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col bg-white">
                    <div className="h-16 flex items-center justify-end px-6 border-b border-gray-100">
                        <button
                            onClick={onClose}
                        ...
                        <div className="max-w-2xl mx-auto">
                            ...
                            {activeTab === 'profile' && (
                                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    ...
                                    <div className="pt-6 flex justify-end">
                                        <button onClick={handleSaveProfile} disabled={isSavingProfile} className="px-6 py-2.5 bg-primary-500 text-white rounded-xl font-bold hover:bg-primary-600 transition-colors shadow-lg shadow-primary-500/20 flex items-center gap-2 disabled:opacity-50">
                            ...
                            {activeTab === 'company' && (
                                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    ...
                                    <div className="pt-6 flex justify-end">
                                        <button onClick={handleSaveCompany} disabled={isSavingCompany || isLoadingCompany} className="px-6 py-2.5 bg-primary-500 text-white rounded-xl font-bold hover:bg-primary-600 transition-colors shadow-lg shadow-primary-500/20 flex items-center gap-2 disabled:opacity-50">
                                ...
                                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    ...
                                    <div className="p-5 border border-gray-200 rounded-2xl bg-gray-50">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <button className="px-6 py-2 bg-gray-900 text-white rounded-xl font-semibold hover:bg-gray-800 transition-colors mt-2">
                                                Atualizar Senha
                                            </button>
                                        </div>
                                    </div>

                                    <div className="p-5 border border-red-200 rounded-2xl bg-red-50/50">
                                        ...
                                        <button className="px-4 py-2 border border-red-300 text-red-600 bg-white hover:bg-red-50 font-medium rounded-lg transition-colors text-sm">
                                            Desativar Conta
                                        </button>
                                    </div>
                                </div>
